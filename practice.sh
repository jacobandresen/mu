...
_refresh_readme_results() {
  ...
  tmp=$(mktemp /tmp/practice-readme.XXXXXX)
  {
    ...
    # Sort by numeric problem index (p1..p10, not the string order that puts p10
    # before p2): emit the integer after 'p' as a sort key, then strip it.
    awk -F'|' '{ n=$2; sub(/^p/,"",n); sub(/-.*/,"",n)
