...
# Rewrite the auto-generated DOJO-RESULTS block in README.md so the front door
# always shows the LAST round's measured per-problem result. Reads "outcome|pid"
# lines for a single round from $1; only the marked region is replaced, the
# curated table and challenges in README are left untouched. README must always
# reflect just the most recent round, so this overwrites (never accumulates).
_refresh_readme_results() {
  local rfile="$1" rnum="$2" tmp
  [ -f README.md ] || return 0
  [ -s "$rfile" ] || return 0
  tmp=$(mktemp /tmp/practice-readme.XXXXXX)
  {
    printf '<!-- DOJO-RESULTS:START (auto-generated by practice.sh — do not edit by hand) -->\n'
    printf '_Last round: %s · model: %s_\n\n' "$(date -Iseconds)" "${MU_AGENT_MODEL:-recommended}"
    printf '| Problem | Result |\n|---|---|\n'
    # One row per problem id (a problem like p10 can spawn several sessions via
    # architect mode — collapse them, worst outcome wins). Sort by numeric index
    # (p1..p10, not the string order that puts p10 before p2).
    awk -F'|' '
      { pid=$2; tot[pid]++; if ($1=="success") ok[pid]++; else bad[pid]=toupper($1) }
      END {
        for (p in tot) {
          n=p; sub(/^p/,"",n); sub(/-.*/,"",n)
          res = (ok[p]==tot[p]) ? "PASS" : bad[p]
          printf "%d\t| %s | %s |\n", n+0, p, res
        }
      }' "$rfile" | sort -n | cut -f2-
...
for round in $(seq 1 "$ROUNDS"); do
  ...
  failed_ids=$(mktemp)
  # Per-round "outcome|pid" results — README shows ONLY this (the last round),
  # while results_all keeps the cumulative log for the end-of-run stdout summary.
  round_results=$(mktemp)
  ...
  _refresh_readme_results "$round_results" "$round"
  rm -f "$round_results"
  ...
  if [ -z "${SKIP_AUTOCOMMIT:-}" ] && git rev-parse --git-dir >/dev/null 2>&1; then
    ...
    if [ -f README.md ] && ! git diff-index --quiet HEAD -- README.md 2>/dev/null; then
      COMMIT_FILES+=(README.md)
    fi
    if [ ${#COMMIT_FILES[@]} -gt 0 ]; then
      git commit -o "${COMMIT_FILES[@]}" \
        -m "dojo round $round: update README.md, CHALLENGES.md, token_usage.md (mu $MU_VER)" \
        >/dev/null || true
    fi
  fi
