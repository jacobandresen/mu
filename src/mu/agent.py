...
* **Learning element** (``reflect``, ``enrich``) — distills the critic's
  feedback across episodes into the knowledge base (``docs/challenges/README.md``).
...
def _load_challenges_for_planner() -> str:
    """Return the Open section of project docs/challenges/README.md, or '' if unavailable."""
    try:
        text = Path('docs/challenges/README.md').read_text(encoding='utf-8')
    except OSError:
        return ''
    if '## Open' not in text:
        return ''
