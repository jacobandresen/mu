...
(``docs/challenges/README.md``) and surfaces lessons that are semantically relevant to the
current goal. This is an ASK operation on the knowledge base, in contrast to
``reflect`` which TELLs it.

The retriever uses sentence-transformers to embed docs/challenges/README.md entries plus a
...
def open_challenges(challenges_path: str = 'docs/challenges/README.md') -> list[str]:
    """Parse the Open section of docs/challenges/README.md into one string per entry."""
    try:
...
def lessons_for(goal: str, k: int = 3,
                challenges_path: str = 'docs/challenges/README.md') -> list[str]:
