...
- **`pytest` in C/Go/Rust Makefile `test:` target** — model writes `test:\n\tpytest` in compiled-language Makefiles; make fails when there are no Python test files, and `clean:` preceding `test:` deletes the binary first. Fixed: `fix_makefile_pytest_in_non_python` reflex replaces pytest with `@true` and removes `clean` from build target prerequisites when no `.py` files exist; makefile-writer skill updated (commit `50589e0`).
- **`MSB1011`: multiple .csproj files — dotnet can't determine which to build** — C# projects with multiple project files cause `dotnet test` to fail without specifying which .csproj to use. This can happen when the model creates both a main project and a test project. Still open — `normalize_test_command` runs at plan creation (before files exist) so can't detect multiple .csproj; needs repair-loop detection to pick the test project.
