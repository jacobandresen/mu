...
- `CS0246: The type or namespace name 'X' could not be found` — missing `using` directive. Add `using System;`, `using System.Collections.Generic;`, or whichever namespace `X` belongs to. When `MU_LSP=all` is active, the Roslyn language server (`Microsoft.CodeAnalysis.LanguageServer`) applies `source.addImport` code actions automatically and resolves most CS0246 errors without manual edits — do not duplicate what Roslyn already handles.
...
- **Roslyn LSP** (`MU_LSP=all`): when enabled, `Microsoft.CodeAnalysis.LanguageServer` (net10) runs after each build attempt and applies LSP code actions. It reliably fixes CS0246 (missing `using`) via `source.addImport`. It does NOT fix CS0103 (`'X' does not exist in the current context`) — that is a semantic/model error requiring a code rewrite, not an import.
