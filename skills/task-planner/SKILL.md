...
- **Rust programs MUST use `cargo test` or `cargo run`** — NEVER `pytest`, `python`, or any non-Rust runner. For a Rust project the Test Command is always `cargo test` (when there are tests) or `cargo run` (when the goal only asks to print output). The `## Test Command` section is REQUIRED — do NOT omit it for Rust projects.
