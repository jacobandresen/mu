...
- **C# programs: use classic style with explicit class Program** — always wrap the entry point as `class Program { static void Main(string[] args) { ... } }`. Do NOT mix top-level statements (CS8803) with class/namespace declarations. All `using` directives must be at the very top of each file.
- **C# console programs: Test Command is `dotnet run`** — for goals that do NOT mention "api", "endpoint", "http", "web", or "test", the correct test is to run the program and check its output: use `dotnet run` (not `dotnet test`, not `dotnet test --filter ...`). Do NOT add a separate test project or xUnit test class for a simple console app.
- **Prefer stdlib over ORMs for SQLite goals**: if the goal says "SQLite" or "sqlite" without mentioning an ORM (SQLAlchemy, peewee, etc.), use Python's built-in `sqlite3` module directly — no `models.py`, no ORM dependency. Never use `flask_sqlalchemy`, `SQLAlchemy`, or any ORM. One implementation file is enough.
