...
- **Flask + SQLite: use the `sqlite3` stdlib module, not Flask-SQLAlchemy.** Flask-SQLAlchemy is a heavy ORM requiring `pip install flask-sqlalchemy`. For any Flask + SQLite task, use `import sqlite3` and `sqlite3.connect(...)` directly. Never add `flask-sqlalchemy` or `flask_sqlalchemy` to `requirements.txt` unless explicitly requested. The `sqlite3` module is built into Python and requires no install.
