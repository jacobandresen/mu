...
- **C/Go/Rust Makefiles must NOT use `pytest` or `python`.** For compiled-language projects with no Python source files, the `test:` recipe must run the compiled binary (`./main`) or language tool (`go test ./...`, `cargo test`). NEVER write `pytest` in a C, Go, or Rust Makefile.
- **Never put `clean` as a prerequisite alongside compile/test targets.** `target: main clean test` runs `clean` which deletes the binary BEFORE the test can run. Either omit `clean` from build prerequisites entirely, or put it as a last step.
