#!/usr/bin/env python
"""Analyze the MSB1003 redirect A/B arms → verdict (plan Step 0.5 backend_build).
...
def main():
    p10_on, p10_off = load('abl_msb_p10_on.json'), load('abl_msb_p10_off.json')
    p4_on, p4_off = load('abl_msb_p4_on.json'), load('abl_msb_p4_off.json')

    out = ['# MSB1003 redirect A/B — plan Step 0.5 backend_build\n',
           'model `qwen2.5-coder-7b-instruct` · level L0 (I3) · '
           'ON = fix (HEAD) · OFF = parent `plan.py`\n']
    ...
    if keep1 is None or keep2 is None:
        ...
        if keep1 and keep2:
            out.append('\n**CREDITED — redirect lifts backend_build; record efficacy Δ, '
                       'bottleneck moves to the next layer.**')
        elif keep2:
            out.append('\n**RETAINED (no-regret) — MSB1003 cleared but backend_build did not '
                       'clear the CI bar: a downstream error (CS0101/CS0246) re-binds it '
                       '(expected, S2 lesson). Re-scan the new first-error to aim the next '
                       'lever / Phase 1 probe.**')
        else:
            out.append('\n**REVERT — p4 control regressed; the redirect harms a passing '
                       'dotnet problem.**')

    text = '\n'.join(out) + '\n'
    (MU / 'abl_msb_verdict.md').write_text(text)
    print(text)
