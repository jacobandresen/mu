#!/usr/bin/env python
"""Analyze the S2 re-evaluation arms → KEEP/DROP (plan Step 0.3, post-MSB1003-fix).
...
machinery as .mu/abl_analyze.py (S2).
"""
import json
import random
...
def main():
    p10_on, p10_off = load('abl_s2b_p10_on.json'), load('abl_s2b_p10_off.json')
    p4_on, p4_off = load('abl_s2b_p4_on.json'), load('abl_s2b_p4_off.json')

    out = ['# S2 re-evaluation (post-MSB1003-fix) — plan Step 0.3\n',
           'model `qwen2.5-coder-7b-instruct` · level L0 (I3) · '
           'ON = MU_S2_TYPE_REFLEXES=1 · OFF = unset · entry-point lever held off\n']

    keep1 = keep2 = None
    gate1_lo = None

    if p10_on and p10_off:
        out.append('\n## p10-dotnet-vue-blog — per-layer q̂ (ON = S2 type reflexes)\n')
    ...
    if p4_on and p4_off:
        ...
        strict2 = lo >= -0.05
        out.append('\n## p4-fibonacci (dotnet control; S2 never fires single-run) — pass-rate\n')
    ...
    if keep1 is None or keep2 is None:
        ...
        # The fix is no-regret and retained regardless (prereg); the gates decide credit.
        if keep1 and keep2:
            out.append('\n**KEEP — S2 lifts backend_build now that CS0101 is reachable; '
                       'flip MU_S2_TYPE_REFLEXES on by default, record efficacy Δ.**')
        elif keep2:
            out.append('\n**STAYS OPT-IN — S2 no significant backend_build lift: CS0246 '
                       '(no Program entry) still binds the runs S2 cannot touch. Build the '
                       'entry-point lever (MU_ASPNET_ENTRYPOINT) first, then re-test S2.**')
        else:
            out.append('\n**REVERT — p4 control regressed (unexpected; S2 should not fire '
                       'on a single-run problem).**')

    text = '\n'.join(out) + '\n'
    (MU / 'abl_s2b_verdict.md').write_text(text)
    print(text)
