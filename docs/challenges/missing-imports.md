...
- **LSP ([`lsp.py`](../../src/mu/lsp.py), `MU_LSP`)** — the strongest generalizer for *this* class: a language server resolves the missing import for **any** symbol, not the fixed set the regex reflexes hard-code. `csharp-ls` add-using is verified (fixes CS0246 by importing the namespace, build passes); `pyright`/`gopls` `source.organizeImports` add the missing module name-aware. The reflexes stay the fast no-server default; LSP extends them under `MU_LSP=1` (gopls) / `MU_LSP=all` (csharp-ls for the p10 CS0246, pyright).
